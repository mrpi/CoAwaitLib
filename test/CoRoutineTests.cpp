...
class MultiBufferPoolsMemoryResource : public boost::container::pmr::memory_resource
{
 ...
 public:
   MultiBufferPoolsMemoryResource(boost::container::pmr::memory_resource* upstream =
...
class CheckAllocAndDealloc : public boost::container::pmr::memory_resource
{
 ...
 public:
   CheckAllocAndDealloc(boost::container::pmr::memory_resource* upstream =
                            boost::container::pmr::get_default_resource())
    : mUpstream(upstream)
   {
   }
...
TEST_CASE("co::Routine: await sleep on boost::asio::io_context")
{
   using namespace std::literals;

   boost::asio::io_context ios;
   boost::optional<boost::asio::io_context::work> iosWork{ios};
   ...
   co::Routine coro{[&ios]() {
      ...
      // there is only one thread executing io_context::run()
...
TEST_CASE("co::Routine: await sleep on boost::asio::io_context directly with chrono type")
{
   using namespace std::literals;
...
TEST_CASE("co::Routine: await boost::asio::io_context (switch to a thread that is executing "
          "io_context::run())")
{
   boost::asio::io_context ios;
   boost::optional<boost::asio::io_context::work> iosWork{ios};
