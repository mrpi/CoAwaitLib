...
struct TestObject
{
   ...
   TestObject(int& destructorCalls) : mDestructorCalls(&destructorCalls) {}

   TestObject(const TestObject&) = delete;
   TestObject& operator=(const TestObject&) = delete;

   ~TestObject() { (*mDestructorCalls)++; }
};
...
SCENARIO("use of co::Routine::SpecificPtr")
{
   GIVEN("a default constructed co::Routine::SpecificPtr")
   {
      ...
      WHEN("get() is call from an ordinary thread")
      {
         ...
         THEN("the result should be nullptr") { REQUIRE(val == nullptr); }
      }
      ...
      WHEN("the pointer is reset() to an object from an coroutine")
      {
         ...
         co::Routine{[&]() {
            ...
            THEN("get() should return the raw pointer to this object when called from the same "
                 "thread")
            {
               REQUIRE(ptr.get() == testObjMainThread);
            }
